...
## Running the code
Final+Notebook.py contains the executable code. The script will call suggest() function which maps all the outputs from different models together. This file alone (with required data) is enough to make predicitons, all the other notebooks are there only to show data preprocessing and training of models. You can adjust suggest() function and refer to the comments to know more about the parameters of function. The output of this function call will be information about five recommended books and their cover image.
